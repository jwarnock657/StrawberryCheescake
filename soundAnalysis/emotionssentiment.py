...
class CNN:
    def __init__(self, model_h5, model_json):
        ...
        # Text
        json_model = os.path.join("soundAnalysis", "saved_models", "WordModel.json")
        weights_model = os.path.join("soundAnalysis", "saved_models", "WordsModel.h5")

        opt = keras.optimizers.rmsprop(lr=0.00001, decay=1e-6)
        json_file = open(json_model, 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        self.loaded_model_text = model_from_json(loaded_model_json)

        self.loaded_model_text.load_weights(weights_model)
        self.loaded_model_text.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy'])

        self.loaded_model_text.predict(np.zeros((1, 200)))
    ...
    def analyzer(self, fileName):
        ...
        with open(os.path.join("soundAnalysis", "voiceAnalysis.csv"), "w") as csvfile:
    ...
    def textAnalysis(self, text):
        ...
        sentiment = self.loaded_model_text.predict(twt, batch_size=16, verbose=2)[0]
        if (np.argmax(sentiment) == 0):
            print("negative", text, time.time() - t1)
        elif (np.argmax(sentiment) == 1):
            print("positive", text, time.time() - t1)

        with open(os.path.join("soundAnalysis", "textAnalysisOutput.txt"), "w") as file:
            file.write(str(np.argmax(sentiment)))

    def recognize_speech(self, recognizer):
        audio_input = sr.AudioFile(os.path.join("soundAnalysis", 'output0.wav'))
    ...
    def speech_recog(self):
        ...
        if user_said["transcription"]:
            ...
            # write said to csv
            fourSSpeech = os.path.join("soundAnalysis", '4sSpeech2Txt.txt')
    ...
    def workerSound(self, num):
        ...
        RECORD_SECONDS = 4
        WAVE_OUTPUT_FILENAME = os.path.join("soundAnalysis", "output{}.wav".format(num))
